...
def single_dose_simulation(num_comp, simulation_time: any, time_unit: str, dose_mg, c: int = 1, plt_stp: float = 0.1):

    sim_time = time_for_single_dose(simulation_time, time_unit)

    y0 = np.concatenate([[dose_mg[0]], np.zeros(num_comp - 1)])

    if type(model_parameters()) != tuple:
        par = tuple([model_parameters()])

    else:
        par = model_parameters()

    y = odeint(my_model, y0, sim_time, par)
...
def multi_dose_simulation(num_comp, simulation_time: any, time_unit: str, num_dose: int, interval: any, dose_mg, c: int = 1,
                          plt_stp: float = 0.1):
    ...
    if len(dose_mg) == 1:
        ...
        while j < len(sim_time):
            ...
            y_n = odeint(my_model, np.concatenate([[dose_mg[0] + yy[:, 0][-1]], init_i]), sim_time[j][1:], par)
            yy = y_n
            j += 1
        ...
        while j < len(sim_time):
            ...
            y_n = odeint(my_model, np.concatenate([[dose_mg[j] + yy[:, 0][-1]], init_i]), sim_time[j][2:], par)
            yy = y_n
            j += 1
...
def multi_dose_sim_delay(num_comp, simulation_time: any, time_unit: str, num_dose: int, interval, dose_mg, delay, c: int = 1,
                         plt_stp: float = 0.1):
    ...
    if len(dose_mg) == 1:
        ...
        while j < len(delay):
            ...
            y_n = odeint(my_model, np.concatenate([[dose_mg[0] + yy[:, 0][-1]], init_i]), new_time[j][1:], par)
            yy = y_n
            j += 1
        ...
        while j < len(delay):
            ...
            y_n = odeint(my_model, np.concatenate([[dose_mg[j] + yy[:, 0][-1]], init_i]), new_time[j][1:], par)
            yy = y_n
            j += 1
