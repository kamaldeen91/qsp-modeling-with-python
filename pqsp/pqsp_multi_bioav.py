...
class MultipleDoseVaryBioav():
    def __init__(self, model, parameters_no_bioav, number_of_compartments, number_of_dose, interval, bioav=None):
    ...
    def simulation(self, simulation_time: any, time_unit: str, dose_mg, compartment_pos=None):
        ...
        time, conc = multi_dose_simulation_vary_bioav(self.model, self.parameters_no_bioav, self.number_of_compartments,
                                                      simulation_time, time_unit, self.number_of_dose, self.interval,
                                                      dose_mg, self.bioav, compartment_pos)
    ...
    def multi_dose_vary_bioav_plot(self, simulation_time, time_unit: str, drug_doses, compartment_pos,
                        yunit: str = 'ng/l', figsize: tuple = (10, 6), show_hstep: bool = False):

        multi_dose_vary_bioav_plot(self.model, self.parameters_no_bioav, self.number_of_compartments, drug_doses,
                                   simulation_time, time_unit, self.number_of_dose, self.interval, self.bioav,
                                   compartment_pos, yunit, figsize, show_hstep)
